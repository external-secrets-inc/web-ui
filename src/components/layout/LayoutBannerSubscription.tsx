...
export function LayoutBannerSubscription() {
  ...
  useEffect(() => {
    if (subscriptions && subscriptions.length > 0) {
      ...
      const updatedData = JSON.parse(
        localStorage.getItem(`banner_${id}`) || "{}"
      );
      ...
      const thresholdDay = [0, ...SHOW_BANNER_THRESHOLDS].find(
        (threshold) => diffDays <= threshold
      );
      if (
        thresholdDay !== undefined &&
        lastDismissedAtThreshold !== thresholdDay
      ) {
        setShowBanner(true);
      }
    }
  ...
  const handleClose = () => {
    const lastDismissedAtThreshold =
      SHOW_BANNER_THRESHOLDS.find((day) => diffDays <= day) || null;
    const updatedData = {
      expiryDate: subData.expiryDate,
      lastDismissedAtThreshold,
    };
  ...
  const expiryDateUS = formatDate(subData.expiryDate, {
    format: "americanDate",
  });
  ...
  return (
    <>
      <Alert
        ...
        className={cn(
          "flex items-center justify-between w-[stretch] m-4",
          !showBanner && "hidden"
        )}
      >
        <div className="flex items-center space-x-2">
          ...
          {diffDays > 0 ? (
            <AlertDescription>
              ...
              <span className="font-semibold">{expiryDateUS}</span>.
            </AlertDescription>
          ) : diffDays === 0 ? (
            <AlertDescription>
              Your subscription expires{" "}
              <span className="font-semibold">today</span>! Please make your
              payment to continue using our services.
            </AlertDescription>
          ) : (
            <AlertDescription>
              Your subscription has expired on{" "}
              <span className="font-semibold">{expiryDateUS}</span>. Please make
              your payment to continue using our services.
            </AlertDescription>
          )}
        </div>
        {diffDays > 0 && (

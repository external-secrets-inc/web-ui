...
export function SecretStoreDataTable() {
  ...
  const columns = useMemo(() => defineColumns<SecretStoreTableData>(columnHelper => [
    ...
    columnHelper.display({
      ...
      cell: props => (
        <div className='flex justify-end'>
          {(props.table.options.meta as SecretStoreTableMeta)?.renderRowActions?.(props.row.original)}
        </div>
      )
    })
  ]), []);
  ...
  const { mutate: deleteSecretStore } = useDeleteSecretStore({
    onError: (error: AxiosError<ApiHttpError>) => handleDefaultApiHttpError(error, "Error while trying to delete Secret Store"),
    onSuccess: () => {
      secretStoresRefetch();
  ...
  if (isErrorSecretStores || isRefetchErrorSecretStores) {
    handleDefaultApiHttpError(secretStoresError, "Error while fetching Secret Stores data");
  }

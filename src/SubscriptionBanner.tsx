...
function SubscriptionBanner() {
  ...
  useEffect(() => {
    const fetchSubscription = async () => {
      try {
        ...
        const subscription = subscriptions.find((sub) => sub.id);

        if (subscription) {
          setSubscriptionId(subscription.id);
          const expiryDate = new Date(subscription.expiryDate);
          calculateDaysUntilExpiration(expiryDate); // Implement this function
        }
      } catch (error) {
  ...
  // calculating days left until subscription runs out
  ...
  return (
    <div
      className={`mx-5 md:mx-14 flex justify-center items-center ${
        isFadingOut ? "animate-fade-out" : ""
      }`}
    >
      <Alert
        variant={threshold === 0 ? "destructive" : "warning"}
        className="mt-6 w-full max-w-[1380px] flex justify-between items-center pl-4 pr-6"
      >
        <div className="flex flex-row space-x-4">
          <svg
            ...
            strokeWidth="1.5"
            stroke="currentColor"
            className="w-16 h-14 md:w-5 md:h-5"
          >
            <path
              strokeLinecap="round"

...
export async function createManifestToken(id: string, options: Partial<ApiWrapperOptions> = {}) {
  const headers = await getAuthHeaders();
  return apiWrapper(async () => {
    const response = await axiosInstance.post(`/api/agents/${id}/manifest-token`, {}, { headers });
    return response.data.token;
  }, { defaultError: 'Failed to create manifest token', ...options });
}

...
const getWorkflowRun = async (signal: AbortSignal, payload: GetWorkflowRunPayload): Promise<WorkflowRunData> => {
  ...
  const response = await axiosInstance.get(`/api/v1/workflowruns/${payload.namespace}/${payload.name}`, {
    headers,
    signal,
    backend: 'ESO_SERVER'
  });
...
const useGetWorkflowRun = (
  ...
  return useQuery({
    queryKey: ["workflows", "useGetWorkflowRun", `useGetWorkflowRun/${payload.namespace}/${payload.name}`],

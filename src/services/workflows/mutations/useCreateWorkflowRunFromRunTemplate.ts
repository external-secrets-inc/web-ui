...
const createWorkflowRunFromRunTemplate = async (
  payload: CreateWorkflowRunFromRunTemplatePayload
): Promise<void> => {
  const headers = await getAuthHeaders();
  await axiosInstance.post(
    `/api/v1/workflowruntemplates/${payload.runTemplateNamespace}/${payload.runTemplateName}/run`,
    payload,
    {
      headers,
      backend: "ESO_SERVER",
...
const useCreateWorkflowRunFromRunTemplate = (
  options?: Omit<
    UseMutationOptions<
      ...
      CreateWorkflowRunFromRunTemplatePayload
    >,
    "mutationFn"
  >
) => {
    ...
    onSuccess: (data, variables, context) => {
      queryClient.invalidateQueries({
        queryKey: [
          ...
          `${variables.runTemplateNamespace}/${variables.runTemplateName}`,
        ],
      });
